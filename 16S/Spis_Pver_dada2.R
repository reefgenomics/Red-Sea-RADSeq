library(dada2)
#setwd("~/Documents/Bioinformatics_scripts/R_scripts/Spis_Pver/dada2_test/")
### ### ### ### ### 
### Read files  ### 
### ### ### ### ###  
cat("Reading files")
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(245,220),maxN=0, maxEE=c(2,2), truncQ=10, rm.phix=TRUE, compress=TRUE, multithread=TRUE) # 
head(out)
...
write.table(asv.final, "Spis_Pver_ASV_table.txt",  quote = FALSE)
write.table(track, "Spis_Pver_ASV_stats.txt", quote = FALSE)
